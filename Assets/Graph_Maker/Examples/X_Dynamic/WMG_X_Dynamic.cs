...
//		graph.axesType = WMG_Axis_Graph.axesTypes.II;
	...
	IEnumerator axesTests() {
		...
		WMG_Anim.animVec2(()=> graph.theOrigin, x=> graph.theOrigin = x, animDuration, new Vector2(graph.theOrigin.x, graph.yAxis.AxisMaxValue));

		if (!noTestDelay) yield return waitTime;
		graph.axesType = WMG_Axis_Graph.axesTypes.AUTO_ORIGIN_Y;
		graph.yAxis.AxisUseNonTickPercent = true;
		WMG_Anim.animVec2(()=> graph.theOrigin, x=> graph.theOrigin = x, animDuration, new Vector2(graph.xAxis.AxisMaxValue, graph.theOrigin.y));

		if (!noTestDelay) yield return waitTime;
		graph.axesType = WMG_Axis_Graph.axesTypes.AUTO_ORIGIN;
		WMG_Anim.animVec2(()=> graph.theOrigin, x=> graph.theOrigin = x, animDuration, new Vector2(graph.xAxis.AxisMaxValue / 4, graph.yAxis.AxisMaxValue / 2));
